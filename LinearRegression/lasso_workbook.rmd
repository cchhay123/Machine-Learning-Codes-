...
```{r}
feat_vars_1 <- names(house_price[, 4:12])
vis_miss(house_price[, feat_vars_1])

t_bins <- bin(house_price$House.Price, nbins = 6, method = "length")
# Bin response variable
plot_dat <- cbind.data.frame(t_bins, house_price[, feat_vars_1])
gg_miss_fct(x = plot_dat, fct = t_bins) +
  labs(x = "Life Expectancy")
```
...
```{r}
imputed_values <- mice( data = house_price[, feat_vars_1], # Set dataset
                        m = 1, # Set number of multiple imputations
                        maxit = 40, # Set maximum number of iterations
                        method = "cart", # Set method
                        print = FALSE)
house_price[,feat_vars_1] <- complete(imputed_values, 1) 
summary(house_price[, feat_vars_1])
...
```{r}
use_dat_1 <- house_price[, c(3:12)] # Select the columns to use for analysis
fit_2 <- lm(House.Price ~.,
            data = use_dat_1)
# Summarise regression
summary(fit_2)
```
...
Wages, Exchange.rates, GDP, Health.Spending, Unemployment.rate at .01% significant level 

* Run a lasso regression with housing price as the response variable.
```{r}
x_vars <- scale(use_dat_1[ ,-1])
fit_3 <- glmnet(x = x_vars, # Fit explanatory variables
                y = use_dat_1$House.Price, # Fit response variable
                alpha = 1, # Set alpha as 1 for lasso
                lambda = 0.5) # Set lambda as 0.5
coef(fit_3)
```
