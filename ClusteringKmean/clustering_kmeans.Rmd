...
We see that for both offensive and defensive data we have 68 samples, 1 variable which corresponds to team name and 19 variables corresponding to explanatory variables. We can view a summary of this data by running `summary()`:

```{r}
summary(off_dat) # Summarize offensive data
```

From this we see that we have no missing values in the data. The variables correspond to two different play types:
...
From this we can see that cluster 3 passed the ball very frequently but struggled at both passing and rushing, cluster 1 ran the ball very frequently and rarely passed the ball, cluster 2 was successful at both passing and rushing, cluster 4 ran the ball frequently but struggled at both passing and rushing. 
...
From this it looks like many of the better teams are contained in clusters 2 and 5.
...
From this we see that the silhouette plot for the first clustering has issues with  cluster 1. For the clustering with k=6 both cluster 2 and 3 appears to have samples that do not belong in that cluster. 
...
Here we are looking for clusters that are major outliers. When k=4, cluster 2 has a lot less points than the other clusters and would warrant investigation. From our previous print out of the teams in each cluster we can see that this cluster corresponds to the military academies who may have a distinct play style. The corresponding clusters for these teams when k=6 is 4 and 1. 
...
When we have k=4, cluster 1 and 2 have a similar within cluster sum of squares while there is a far smaller value for cluster 2 with the military academies and cluster 4. When k=6, clusters 1 and 4 have a a quite low value for within cluster sum of squares since they only contain one and two samples respectively. 
...
Lets check the distance between some teams to see how this looks. Lets focus on Georgia, Michigan, Notre, Stanford, and Army. We would expect that Georgia and Alabama are quite similar, that Notre Dame and Stanford are quite similar, that Notre Dame is reasonably similar to Georgia and that Army is quite distinct from the other teams. 

```{r Check distance}
cat("Georgia v Michigan")
dist(off_dat[off_dat$teams %in% c("Georgia", "Michigan" ),2:20])[1]
cat("Georgia v Notre Dame")
dist(off_dat[off_dat$teams %in% c("Georgia", "Notre Dame" ),2:20])[1]
cat("Georgia v Army")
dist(off_dat[off_dat$teams %in% c("Georgia",  "Army"),2:20])[1]
...
k_4_sol <- cbind.data.frame(off_dat$teams[off_dat$teams %in% c("Michigan", "Army", "Georgia",
                                                               "Notre Dame", "Stanford")],
                            clusters_1[off_dat$teams %in% c("Michigan", "Army", "Georgia",
                                                               "Notre Dame", "Stanford")])
...
k_6_sol <- cbind.data.frame(off_dat$teams[off_dat$teams %in% c("Michigan", "Army", "Georgia",
                                                               "Notre Dame", "Stanford")],
                            clusters_2[off_dat$teams %in% c("Michigan", "Army", "Georgia",
                                                               "Notre Dame", "Stanford")])
# Set names on new dataset
...
These look pretty good, however, it is interesting to note that Georgia and Notre Dame end up in different clusters for K=6 even though they are quite similar to each other. This may indicate that we have too many clusters in the data if we want Georgia and Notre Dame to be in the same cluster, or that there are too few if we expect Notre Dame and Georgia/Michigan to be in different clusters. 
...
From the gap statistic, we get an optimum number of clusters as 1, which does not work well for analysis. Often the choice of optimal cluster number is a combination of both statistical results and analysis requirements. We will proceed with k as 2, using the silhouette plot. 

```{r Run K-means 3}
set.seed(12345) # Set seed for reproducibility
fit_3 <- kmeans(x = off_dat_2[2:20], # Set data as explantory variables 
...
From this we see that cluster 1 has superior performance for both passes and rushes. 
...
The guidance is to choose the w values which results in largest gap statistic, the algorithm will try multiple values and return the value which leads to the best gap statistic. However, this leads to all non-zero coefficients, let's instead choose the lowest value within one standard deviation, 2.0315. We can then use this in our clustering algorithm:

```{r}
# Run sparse K-means with selected value of tuning parameter
km.sparse <- KMeansSparseCluster(x = off_dat_2[2:20], # Set data
                                 K=2, # Set cluster number
                                 wbounds =  2.03151, # Set tuning parameter
                                 nstart = 25, # Set number of starts
